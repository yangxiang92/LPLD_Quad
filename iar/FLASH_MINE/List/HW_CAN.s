...
//   88   OSC->CR |= OSC_CR_ERCLKEN_MASK | OSC_CR_EREFSTEN_MASK;
        LDR.W    R0,??DataTable19_2  ;; 0x40065000
        LDRB     R0,[R0, #+0]
        ORRS     R0,R0,#0xA0
        LDR.W    R1,??DataTable19_2  ;; 0x40065000
        STRB     R0,[R1, #+0]
//   89   
//   90   if(canx == CAN0)
...
//   99       PORTB->PCR[18] = PORT_PCR_MUX(2)| PORT_PCR_PE_MASK | PORT_PCR_PS_MASK; 
        LDR.W    R0,??DataTable19_6  ;; 0x4004a048
        MOVW     R1,#+515
        STR      R1,[R0, #+0]
        B.N      ??LPLD_CAN_Init_4
//  100     }
...
??LPLD_CAN_Init_3:
        LDR.W    R0,??DataTable19_7  ;; 0x40049030
        MOV      R1,#+512
        STR      R1,[R0, #+0]
//  104     }
//  105 
...
//  108       PORTB->PCR[19] = PORT_PCR_MUX(2)| PORT_PCR_PE_MASK | PORT_PCR_PS_MASK; 
        LDR.W    R0,??DataTable19_8  ;; 0x4004a04c
        MOVW     R1,#+515
        STR      R1,[R0, #+0]
        B.N      ??LPLD_CAN_Init_6
//  109     }
...
??LPLD_CAN_Init_5:
        LDR.W    R0,??DataTable19_9  ;; 0x40049034
        MOV      R1,#+512
        STR      R1,[R0, #+0]
        B.N      ??LPLD_CAN_Init_6
//  113     }
...
//  119       PORTC->PCR[17] = PORT_PCR_MUX(2)| PORT_PCR_PE_MASK | PORT_PCR_PS_MASK; 
        LDR.W    R0,??DataTable19_10  ;; 0x4004b044
        MOVW     R1,#+515
        STR      R1,[R0, #+0]
        B.N      ??LPLD_CAN_Init_8
//  120     }
...
??LPLD_CAN_Init_7:
        LDR.W    R0,??DataTable19_11  ;; 0x4004d060
        MOVW     R1,#+515
        STR      R1,[R0, #+0]
//  124     }
//  125 
...
//  128       PORTC->PCR[16] = PORT_PCR_MUX(2)| PORT_PCR_PE_MASK | PORT_PCR_PS_MASK; 
        LDR.W    R0,??DataTable19_12  ;; 0x4004b040
        MOVW     R1,#+515
        STR      R1,[R0, #+0]
        B.N      ??LPLD_CAN_Init_6
//  129     }
...
??LPLD_CAN_Init_9:
        LDR.W    R0,??DataTable19_13  ;; 0x4004d064
        MOVW     R1,#+515
        STR      R1,[R0, #+0]
//  133     }
//  134   } 
...
//  254   switch(baud)
        ...
        LDR.W    R4,??DataTable19_14  ;; 0x14585
        CMP      R3,R4
        BEQ.N    ??LPLD_CAN_SetBaudRate_0
        LDR.W    R4,??DataTable19_15  ;; 0x186a0
        CMP      R3,R4
        BEQ.N    ??LPLD_CAN_SetBaudRate_0
        LDR.W    R4,??DataTable19_16  ;; 0x1e848
        CMP      R3,R4
        BEQ.N    ??LPLD_CAN_SetBaudRate_0
        LDR.W    R4,??DataTable19_17  ;; 0x3d090
        CMP      R3,R4
        BEQ.N    ??LPLD_CAN_SetBaudRate_1
        LDR.W    R4,??DataTable19_18  ;; 0x7a120
        CMP      R3,R4
        BEQ.N    ??LPLD_CAN_SetBaudRate_1
        LDR.W    R4,??DataTable19_19  ;; 0xf4240
        CMP      R3,R4
...
??LPLD_CAN_EnableIrq_0:
        LDR.W    R0,??DataTable19_21  ;; 0x400a4000
        CMP      R4,R0
...
//  489   if(msg_num_temp >= MSG_MAX_NO || in_data_length > 8)
        UXTB     R6,R6            ;; ZeroExt  R6,R6,#+24,#+24
        CMP      R6,#+16
        BCS.N    ??LPLD_CAN_TransmitMessage_0
        UXTB     R9,R9            ;; ZeroExt  R9,R9,#+24,#+24
        CMP      R9,#+9
        BCC.N    ??LPLD_CAN_TransmitMessage_1
//  490   {
//  491     return 0; // 超出阈值
...
//  612   if(msg_num < 32)
        UXTB     R1,R1            ;; ZeroExt  R1,R1,#+24,#+24
        CMP      R1,#+32
        BCS.N    ??LPLD_CAN_Interrupt_ClearPending_0
//  613   {
...
//  643   if(msg_num < 32)
        UXTB     R1,R1            ;; ZeroExt  R1,R1,#+24,#+24
        CMP      R1,#+32
        BCS.N    ??LPLD_CAN_Interrupt_GetFlag_0
//  644   {
...
//  691   if(msg_num < 32)
        UXTB     R1,R1            ;; ZeroExt  R1,R1,#+24,#+24
        CMP      R1,#+32
        BCS.N    ??LPLD_CAN_EnableMsgInterrupt_0
//  692   {
...
//  922   if( msg_num >= MSG_MAX_NO || in_length >8)
        UXTB     R1,R1            ;; ZeroExt  R1,R1,#+24,#+24
        CMP      R1,#+16
        BCS.N    ??LPLD_CAN_WriteData_0
        UXTB     R2,R2            ;; ZeroExt  R2,R2,#+24,#+24
        CMP      R2,#+9
        BCC.N    ??LPLD_CAN_WriteData_1
//  923   {
//  924       return; 
