...
//                    \LPLD\HW\HW_DMA.c -D LPLD_K60 -lCN                      /
...
// E:\ShawnDocuments\IAR_WorkSpace\LPLD_Quad_V3\source\lib\LPLD\HW\HW_DMA.c
//    1 /**
//    2  * @file HW_DMA.c
...
//   70   //关闭通道x硬件DMA请求 
//   71   DMA0->ERQ &= ~(1<<chx);
        LDR.W    R0,??DataTable18_4  ;; 0x4000800c
        LDR      R0,[R0, #+0]
        MOVS     R11,#+1
        LSLS     R11,R11,R1
        BICS     R0,R0,R11
...
//   74   DMAMUX->CHCFG[chx] = DMAMUX_CHCFG_SOURCE(req);
        ...
        LDR.W    R0,??DataTable18_5  ;; 0x40021000
        LDRB     R11,[SP, #+1]
        ANDS     R11,R11,#0x3F
        STRB     R11,[R1, R0]
//   75   //是否使能周期触发功能
...
//  109     DMA0->TCD[chx].CITER_ELINKNO = DMA_CITER_ELINKNO_CITER(major_cnt);
        ...
        ADD      R0,R0,#+1073741824
        ADDS     R0,R0,#+36864
        LSLS     R11,R2,#+17      ;; ZeroExtS R11,R2,#+17,#+17
        LSRS     R11,R11,#+17
        STRH     R11,[R0, #+22]
...
??LPLD_DMA_Init_5:
        UXTB     R1,R1            ;; ZeroExt  R1,R1,#+24,#+24
        LDR.N    R0,??DataTable18_5  ;; 0x40021000
        LDRB     R0,[R1, R0]
        ORRS     R0,R0,#0x80
        UXTB     R1,R1            ;; ZeroExt  R1,R1,#+24,#+24
        LDR.W    R11,??DataTable18_5  ;; 0x40021000
